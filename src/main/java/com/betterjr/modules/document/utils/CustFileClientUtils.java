...
//        fileItem.setAbsoFile(tmpFile);
     ...
     * @param anFileItem
     */
    public static void fileWechatDownload(final HttpServletResponse response, final CustFileItem anFileItem,final String basePath) {

        fileWechatDownloadPdfToJpg(response, anFileItem,basePath);
    }
    
    public static void fileWechatDownloadPdfToJpg(final HttpServletResponse response, final CustFileItem anFileItem,final String basePath) {
        OutputStream os = null;
        String msg = null;
        try {
            if (anFileItem != null) {
                File file = FileUtils.getRealFile(basePath + anFileItem.getFilePath());
                file=converPdfToImg(file.getPath(),file.getParentFile()+file.getName().substring(0, anFileItem.getFileName().indexOf(".")),"jpg",3f);
                if (file != null) {
     ...
     * @return  转换后图片的文件名集合
     */
    public static File converPdfToImg(String pdfFilePath,String imgPushPath,String toFormat,float imgScaling){
        File file=new File(imgPushPath+"."+toFormat);
        //定义Document,用于转换图片
